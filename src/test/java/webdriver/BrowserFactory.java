...
final public class BrowserFactory {
    ...
    private static RemoteWebDriver getWebDriver(final Browsers type, Proxy proxy) {
        if (Boolean.valueOf(props.getProperty("useSeleniumGrid"))) {
            return setupGrid(type);
        } else {
    ...
    private static RemoteWebDriver setupGrid(final Browsers type) {
        RemoteWebDriver driver = null;
        DesiredCapabilities capabilities = null;
        switch (type) {
            case CHROME:
                capabilities = setUpChromeCapabilities();
                break;
            case FIREFOX:
                capabilities = setUpFirefoxCapabilities();
                break;
            default:
                break;
        }
        try {
            driver = new RemoteWebDriver(new URL(props.getProperty("seleniumGridUrl")), capabilities);
        } catch (MalformedURLException e) {
            Logger.getInstance().fatal("Cannot start browser.");
        }
        return driver;
    }

    private static DesiredCapabilities setUpChromeCapabilities() {
        DesiredCapabilities capabilities = DesiredCapabilities.chrome();
        return capabilities;
    }

    private static DesiredCapabilities setUpFirefoxCapabilities() {
